...
export async function GET(request: NextRequest) {
  try {
    ...
    if (date) {
      ...
      query = query.gte('data_appuntamento', startOfDay.toISOString())
                   .lte('data_appuntamento', endOfDay.toISOString())
    } else if (startDate && endDate) {
      // Date range
      query = query.gte('data_appuntamento', startDate)
      ...
      query = query.gte('data_appuntamento', new Date().toISOString())
...
export async function PUT(request: NextRequest) {
  try {
    ...
    const { error } = await supabase
      .from('appuntamenti')
      .update(updateData)
      .eq('id_appuntamento', id_appuntamento)
      .eq('user_id', session.user.id)
